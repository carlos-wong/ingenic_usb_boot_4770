...
extern int mmc_erase(u32 start, u32 end, u32 erase_all);
...
#define RTC_FREQ_DIVIDER	(32768 - 1)
	...
	/* wite rtc second */
	rtc_write_reg(RTC_RTCSR, second);

	serial_puts("RTC time has set to:");
	serial_put_hex(rtc_read_reg(RTC_RTCSR));
}
...
int NAND_OPS_Handle(u8 *buf)
{
	...
	case NAND_MARK_BAD:
		...
		nand_erase(1, start_addr, 1);	//add by ylyuan
	...
	case NAND_ERASE:
		...
		break;
	case SD_MMC_ERASE:
		dprintf("\n Request : SD_MMC_ERASE!");
		ret_dat = mmc_erase(start_addr, ops_length, option);
		handshake_PKT[0] = (u16) ret_dat;
		handshake_PKT[1] = (u16) (ret_dat>>16);
		HW_SendPKT(1,handshake_PKT,sizeof(handshake_PKT));
		udc_state = IDLE;
		dprintf("\n Request : SD_MMC_ERASE_FINISH!");
		break;
	...
	case NAND_PROGRAM:
		...
		ops_length -= 1;
		if(option == NO_OOB)
			check_len = ops_length * Hand.nand_dpp_without_oob;
		else
			check_len = ops_length * (Hand.nand_dpp_with_oob + Hand.nand_os);

//		serial_puts("\nops_length = ");serial_put_hex(ops_length);
//		serial_puts("\ncheck_len = ");serial_put_hex(check_len);

		/* We could not make sure check_len is align with 4 */
		host_crc = (u8 *)Bulk_out_buf + check_len;		

		file_crc32 = (unsigned int)(*host_crc) | (unsigned int)(*(host_crc + 1)) << 8 | 	\
			 (unsigned int)(*(host_crc + 2)) << 16 | (unsigned int)(*(host_crc + 3)) << 24;
		/* We could not make sure check_len is align with 4 */
	...
	break;
        ...
        case 0x4760:
                ...
                nand_init_4760(Hand.nand_bw, Hand.nand_rc, Hand.nand_ps,                                \
                               Hand.nand_ppb, Hand.nand_bchbit, Hand.nand_eccpos,                       \
                               Hand.nand_bbpos, Hand.nand_bbpage, Hand.nand_force_erase,                \
                               Hand.nand_os, Hand.nand_fs_without_oob, Hand.nand_dpp_without_oob,       \
                               Hand.nand_fs_with_oob, Hand.nand_dpp_with_oob);
